...
public class GroupEventDao {
    ...
    private static GroupEventDao instance = new GroupEventDao();

    private GroupEventDao() {

    }

    public static GroupEventDao getInstance() {
        return instance;
    }
    ...
    public void saveGroupEvents(List<GroupEvent> groupEventList) {
        ...
        try {
            ...
            ALogger.log(ALogger.LogPriority.error, GroupEventDao.class, "Save GroupEvents failed for JSON parse!", e);
        } catch (UnsupportedEncodingException e) {
            ALogger.log(ALogger.LogPriority.error, GroupEventDao.class, "Save GroupEvents failed for Charset error!", e);
        }
    }
    ...
    public List<GroupEvent> getGroupEvents() {
        ...
        try {
            ...
            ALogger.log(ALogger.LogPriority.error, GroupEventDao.class, "Get GroupEvents failed for JSON parse!", e);
        } catch (UnsupportedEncodingException e) {
            ALogger.log(ALogger.LogPriority.error, GroupEventDao.class, "Get GroupEvents failed for Charset error!", e);
        }
        return groupEvents;
    }

    public ASQLMapStorage getStore() {
        return AStoreManager.getInstance().getGroupEventStore();
    }
}
